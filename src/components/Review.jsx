...
const Review = () => {
  ...
  useEffect(() => {
    if (!isTv) {
      fetchFromAPI(
        `movie/${id}/reviews?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=${pageN}`
      ...
      fetchFromAPI(
        `tv/${id}/reviews?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=${pageN}`
      ).then((data) => setReviews(data.results));
  ...
  return (
    <>
      ...
      <Box
        pt={2}
        sx={{
          height: reviews.length <= 1 ? "100vh" : "fit-content",
